...
mensaje = '             '
mensaje = 'Grupo de análisis de datos y modelación matemática - Campus Yucatán UNAM'

mensaje = '             '
mensaje = '¿ Quiénes somos ?

mensaje = '             '
mensaje = 'Somos un grupo conformado por académicos pertenecientes a las entidades de la UNAM en Yucatán. Nuestro objetivo es desarrollar modelos matemáticos y computacionales orientados principalmente a atender problemáticas a nivel regional.'

mensaje = '             '
mensaje = ' Proyecto COVID - 19 '

mensaje = '             '
mensaje = 'Es un esfuerzo dirigido a modelar la evolución de la COVID-19 en el Estado de Yucatán. Este trabajo es un proyecto totalmente académico que presenta resultados de la posible evolución de la pandemia en la región. Utilizamos diferentes modelos matemáticos y datos reales  publicados por las autoridades sanitarias a partir del 13 de marzo de 2020. Este proyecto es sólo informativo y no se busca que estos resultados sean considerados ni reportados como una información confirmada para guiar decisiones clínicas.'
...
fig_sir_j.add_trace(go.Scatter(x=data_sir_j['Fecha'], y= data_sir_j['Infectados'], mode='lines',line_color='orange', name = 'Infectados (predicción)'))
...
fig_sis.add_trace(go.Scatter(x=data_sis_n['Fecha'], y= data_sis_n['activos'], mode='lines',line_color='orange', name = 'Infectados activos (predicción)'))
...
fig_epi.add_trace(go.Scatter(x=modelo_epi['Fecha'], y= modelo_epi['CASOS'], mode='lines',line_color='red', name = 'Casos acumulados reales'))
...
fig_seir.add_trace(go.Scatter(x=seir['Fecha'], y= seir['Expuestos'], mode='lines',line_color='green', name = 'Expuestos reales'))
fig_seir.add_trace(go.Scatter(x=seir['Fecha'], y= seir['Infectados'], mode='lines',line_color='cyan', name = 'Infectados'))
fig_seir.add_trace(go.Scatter(x=seir['Fecha'], y= seir['Recuperados'], mode='lines',line_color='blue', name = 'Recuperados'))
...
figx = go.Figure()
figx.add_trace(go.Scatter(x=fii['FECHA_INGRESO'].values.tolist(),y=fii['ACUMULADO'].values.tolist(), mode='lines+markers', line_color='red', name = 'Casos confirmados'))
figx.add_trace(go.Scatter(x=nii['FECHA_INGRESO'].values.tolist(),y=nii['ACUMULADO'].values.tolist(), mode='lines+markers', line_color='blue', name = 'Casos negativos'))


figx.update_layout(title="Casos en el Estado de Yucatán",xaxis_title="Fecha de ingreso",yaxis_title="Número de personas", title_x=0.5, legend_orientation="h", template = 'plotly_dark')
...
fig_cajas_acumulado.update_layout(title="Modelo SIR (tipo Gompertz - casos acumulados",yaxis_title="Número de personas",title_x=0.43,template = 'plotly_dark')
...
intro = html.Div(children = [html.P('Grupo de análisis de datos y modelación matemática'),
        htlm.P('¿Quiénes somos?'),
        html.P('Somos un grupo conformado por académicos pertenecientes a las entidades de la UNAM en Yucatán. Nuestro objetivo es desarrollar modelos matemáticos y computacionales orientados principalmente a atender problemáticas a nivel regional.'),
        html.P('Proyecto COVID-19'),
